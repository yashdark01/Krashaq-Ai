...
**Design rule (Anthropic):** MCP handles connectivity; Skills handle workflow logic. Never duplicate — MCP says _how to call the API_; Skill says _when, in what order, and how to present results_.
...
_Next step: Approve **AI-1** scope and port `agent_router.py` tools into LangGraph.js on `feat/agentic-ai-1`._
