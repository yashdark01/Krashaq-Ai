...
from app.services.cache_service import get_cache_service, cached_tool
from app.services.metrics import get_metrics_service, calculate_response_quality
...
def classify_with_llm(state: AgentState) -> AgentState:
    ...
    try:
        llm = get_llm_with_fallback(settings.llm_provider if hasattr(get_settings(), 'llm_provider') else "ollama", temperature=0.1)
...
def reflect(state: AgentState) -> AgentState:
    ...
    try:
        llm = get_llm_with_fallback(settings.llm_provider if hasattr(get_settings(), 'llm_provider') else "ollama", temperature=0.3)
