...
from app.conversation.services.irrigation_decision import get_irrigation_decision
...
async def general_response(message: str, location: str = "Delhi", language: str = "hi") -> str:
    ...
    try:
        logger.info("[GENERAL RESPONSE] Importing resolve_llm_with_chain")
        from app.conversation.services.llm_provider import resolve_llm_with_chain
        from app.common.config import get_settings
        
        settings = get_settings()
        logger.info(f"[GENERAL RESPONSE] Initializing LLM with fallback chain")
        llm, provider_used, provider_errors = resolve_llm_with_chain(temperature=0.7)
        logger.info(f"[GENERAL RESPONSE] LLM initialized successfully: {type(llm)}, provider: {provider_used}")
        if provider_errors:
            logger.info(f"[GENERAL RESPONSE] Provider errors: {provider_errors}")
