...
describe('main navigation by role', () => {
  ...
  it('filters bottom nav consistently', () => {
    ...
    expect(getBottomNavForRole('supplier').some((item) => item.href === '/supplier/subscriptions')).toBe(true);
  });

  it('farmer sees subscription nav; supplier sees subscriptions hub', () => {
    expect(getMainNavForRole('farmer').some((item) => item.href === '/farmer/subscription')).toBe(true);
    expect(getMainNavForRole('supplier').some((item) => item.href === '/supplier/subscriptions')).toBe(true);
  });
});
