...
function serializeSupplier(user: Record<string, unknown>, license: Record<string, unknown> | null, farmerCount: number) {
  return {
    id: String(user._id),
...
export async function onboardSupplier(adminId: string, adminName: string, data: SupplierOnboardInput) {
  const users = await getCollection('users');
