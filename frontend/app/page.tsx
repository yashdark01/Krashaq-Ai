...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (user) {
      const userLocation = user?.locality || user?.tehsil || user?.district || user?.state || "Delhi"
      ...
      <MainLayout>
        <div className="h-full flex flex-col p-6 gap-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 shrink-0">
            <WeatherCard weather={weather} onLocationChange={setLocation} location={location} fullLocation={fullLocation} />
            <IrrigationPanel />
          </div>

          {/* Chat Section */}
          <div className="flex-1 min-h-0">
            <ChatInterface location={location} onLocationChange={setLocation} fullLocation={fullLocation} />
          </div>
        </div>
      </MainLayout>
    </ProtectedRoute>
