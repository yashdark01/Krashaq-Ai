...
import { MessageSquare, Users, Bell, KeyRound, Activity, BarChart3 } from 'lucide-react';
...
export function SupplierUsagePanel({
  ...
  const path =
    apiPath ??
    (supplierId ? `/api/admin/suppliers/${supplierId}/usage` : '/api/supplier/analytics');
                ...
                <tbody className="divide-y">
                  {farmers.map((f) => (
                    <tr key={f.farmer_id} className="hover:bg-muted/20">
                      ...
                      <td className="px-3 py-2">
                        <Badge variant={f.active_7d ? 'default' : 'outline'}>{f.messages_7d}</Badge>
                      </td>
