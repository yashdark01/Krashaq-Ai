...
export default function ConfigPanel() {
  ...
  if (loading) {
    return (
      <div className="flex justify-center py-8 text-muted-foreground">Loading configuration…</div>
    );
  }
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <Card>
        ...
        <CardContent className="text-sm space-y-2">
          ...
          <p>
            Gemini model:{' '}
            <code className="text-xs bg-muted px-1 rounded">{config.gemini_model}</code>
          </p>
          <p>
            Agent runtime: <strong>{config.agent_runtime}</strong>
          </p>
          <p>Fallback chain: {config.llm_fallback_chain.join(' → ')}</p>
          <p>
            LangSmith:{' '}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Whether required env keys are present (values are never shown)
          </CardDescription>
        </CardHeader>
