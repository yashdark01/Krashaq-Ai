      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-2">
              <div
                className={`px-3 py-1 rounded-full text-sm inline-block ${getStatusColor(health.mongodb.status)}`}
              >
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-2">
              <div
                className={`px-3 py-1 rounded-full text-sm inline-block ${getStatusColor(health.redis.status)}`}
              >
                {health.redis.status}
              </div>
              <div className="text-sm text-muted-foreground">Latency: {health.redis.latency}ms</div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-2">
              <div
                className={`px-3 py-1 rounded-full text-sm inline-block ${getStatusColor(health.llm_provider.status)}`}
              >
                {health.llm_provider.status}
              </div>
              <div className="text-sm text-muted-foreground">{health.llm_provider.provider}</div>
              <div className="text-sm text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-2">
              <div
                className={`px-3 py-1 rounded-full text-sm inline-block ${getStatusColor(health.twilio.status)}`}
              >
