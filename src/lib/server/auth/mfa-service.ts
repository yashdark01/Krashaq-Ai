...
export async function verifyMfaLogin(mfaToken: string, code: string, meta?: SessionMeta) {
  ...
  const { assertSupplierLicenseAllowsLogin } =
    await import('@/lib/server/services/supplier-license-service');
  const { assertFarmerSubscriptionAllowsLogin } =
    await import('@/lib/server/services/farmer-subscription-service');
