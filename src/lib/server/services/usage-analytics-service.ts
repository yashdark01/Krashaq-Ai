...
export async function getChatUsageByUserIds(
  userIds: string[]
...
export async function getSupplierUsageAnalytics(
  supplierId: string
): Promise<SupplierUsageAnalytics | null> {
  const users = await getCollection('users');
...
export async function getUsageSummariesForSuppliers(
  ...
  Map<
    string,
    Pick<
      SupplierUsageAnalytics['summary'],
      'chat_sessions' | 'total_messages' | 'messages_7d' | 'active_farmers_7d'
    >
  >
> {
