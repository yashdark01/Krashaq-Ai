...
export async function getSubscriptionByFarmerId(farmerId: string) {
  ...
  const docs = await col
    .find({ farmer_id: farmerId } as never)
    .sort({ created_at: -1 })
    .limit(1)
    .toArray();
  const doc = docs[0];
...
export async function getSubscriptionsByFarmerIds(farmerIds: string[]) {
  ...
  const col = await getCollection('farmer_subscriptions');
  const docs = await col
    .find({ farmer_id: { $in: farmerIds } } as never)
    .sort({ created_at: -1 })
    .toArray();
...
export async function reactivateFarmerSubscription(
  ...
  const col = await getCollection('farmer_subscriptions');
  const docs = await col
    .find({ farmer_id: farmerId, supplier_id: supplierId } as never)
    .sort({ created_at: -1 })
    .limit(1)
    .toArray();
  const latest = docs[0];
  ...
  const status =
    validUntil > new Date() ? (latest.plan === 'trial' ? 'trial' : 'active') : 'expired';
...
export async function assertFarmerSubscriptionAllowsLogin(
  farmerId: string,
