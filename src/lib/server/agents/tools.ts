...
export function buildKrashaqTools(ctx: ToolContext) {
  const fetchWeather = tool(
    ...
    },
    {
      name: 'fetch_weather',
      description:
        'Get current weather for a location in India. Use when user asks about mausam, temperature, rain.',
      schema: z.object({
  ...
  const webSearch = tool(
    ...
    },
    {
      name: 'web_search',
      description:
        'Search the web for latest schemes, MSP, market prices, pest alerts, and policy updates.',
  ...
  const analyzeCropNeeds = tool(
    async ({ crop, location }) => {
      ...
      const kb = await searchKbWithCitations(`${cropName} farming`, 2);
      if (kb.chunks.length) {
        parts.push(
          'Knowledge base:\n' +
